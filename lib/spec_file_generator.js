...
const getLockfileName = require('../lib/npm_helpers.js').getLockfileName;
...
function dependenciesToSources(name, deps) {
  ...
  // Prefer the URL the lockfile recorded. npm ci checks every tarball against
  // the lockfile's integrity hash, so a reconstructed URL pointing at
  // different bytes fails with EINTEGRITY far from the cause. Reconstruction
  // remains as a fallback for callers that only supply a name and version.
  const depsOnly = deps.filter(dep => dep.name !== name);
  depsOnly.forEach((dependency, index) => {
    const url = dependency['url'] || npmUrl(dependency['name'], dependency['version']);
    sources.push(`Source${index + 1}: ${url}`);
  });

  sources.push(`Source${depsOnly.length + 1}: ${getLockfileName(getRpmPackageName(name), '%{version}')}`)
  return sources;
}

function sortDependencies(deps) {
  const result = deps.map((dependency) => {
    return {'name': dependency[0], 'version': dependency[1], 'url': dependency[2]}
  });
  result.sort((a, b) => {
    if (a['name'] === b['name']) {
...
function generateSpecFile(npm_module, files, analysis, mainPackageBinaries, release, template_name, use_legacy_peer_deps, licensesFilename) {
  ...
  const replacements = {
    ...
    RELEASE: release === 1 ? '%autorelease' : release + '%{?dist}',
  ...
  if (bundled.length !== 0) {
    const sortedBundled = sortDependencies(bundled.map(d => [d.name, d.version, d.url]));
    ...
    if (licensesFilename) {
      replacements['LICENSES_FILENAME'] = licensesFilename;
    }
  } else {
